...
const Screen4 = ({navigation}) => {
  ...
  const getMentorsDetails = () => {
    setIsLoding(true);
    axios
      .get(`${GW_URL}/mentors`, parentConfig)
      .then(res => {
        ...
        axios
          .get(`${GW_URL}/parents/${parentId}/getSchedule`, parentConfig)
          .then(res => {
            let schduledMentorId =
              res.data.schedules[0]?.mentor_id[
                res.data.schedules[0]?.mentor_id.length - 1
              ];
            mentor.forEach(element => {
